...
const HeaderLink: React.FC<{ item: NavLinkType }> = ({ item }) => {
    ...
    return (
        ...
        >
            <Link
                href={item.href}
                className={`text-lg md:text-xl flex font-normal text-darkblue dark:text-white hover:text-primary dark:hover:text-primary ${
                    item.href === path
                ...
                <ul className="absolute py-2 left-0 mt-0.5 w-60 bg-white dark:bg-white/10 shadow-lg rounded-lg">
                    {item.submenu?.map((subItem, index) => (
                        <li key={index}>
                            <Link
                                href={subItem.href}
                                className="block px-4 py-2 text-base md:text-lg text-darkblue dark:text-white hover:bg-neutral-50 dark:hover:bg-darkmode/10 hover:text-primary dark:hover:text-primary"
                            >
                                {subItem.label}
                            </Link>
                        </li>
