...
const MobileHeaderLink: React.FC<{ item: NavLinkType }> = ({ item }) => {
  ...
  const itemClass = `flex items-center justify-between w-full py-2 text-darkblue dark:text-white focus:outline-none hover:text-primary dark:hover:text-primary hover:cursor-pointer ${
  ...
  return (
    <div className="relative w-full">
      {item.href ? (
        ...
        <button type="button" onClick={() => setSubmenuOpen(!submenuOpen)} className={itemClass}>
          {item.label}
          {item.submenu && (
            <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24">
              <path
