...
const Timeline = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/events/?skip=0&limit=100&month=' + monthToday)
      .then(res => res.json())
      .then(data => {
        setEventList(data)
        setLoading(false)
            ...
            {eventList.map((event, index) => (
              <li key={index}>
                ...
                <div className="mb-6 ms-6 pb-6">
                  <label className="text-sm text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">{new Date(event.start).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</label>
